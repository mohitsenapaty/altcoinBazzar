...
//query.connectionParameters = 'postgres://postgres:postgres@localhost:5432/exchange';
//var db_client = require('./pg_db_client');
...
router.post('/:pwd/', function(req, resp, next){
  ...
  db_client.connect(function (err){
    ...
    db_client.query("SELECT * FROM bitcoin_wallet_test_user WHERE user_id=$1;", [userID], function(err,res)
    {
      if (err){console.log(err); resp.send(login_data);}
      else
      { //console.log(res);
        ...
        if (res.rows.length==1){
          console.log("Bitcoin address is present");
